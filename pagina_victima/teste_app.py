...
# Configuración manual de Flask para pruebas
app = Flask(__name__)
...
# Modelo: debe coincidir con tu tabla real
...
class PruebasUsuario(unittest.TestCase):
    ...
    def tearDown(self):
        with app.app_context():
            db.session.remove()
            # db.drop_all()  # Descomenta si deseas limpiar todo después de cada test

    # 🧪 Prueba 1: Crear un nuevo usuario
    def test_crear_usuario(self):
        with app.app_context():
            nuevo = Usuario(nombre="Test", correo="test@correo.com", contrasena="1234")
            db.session.add(nuevo)
            db.session.commit()
            usuario = Usuario.query.filter_by(correo="test@correo.com").first()
            self.assertIsNotNone(usuario)
            self.assertEqual(usuario.nombre, "Test")
    ...
    def test_usuario_inexistente(self):
        with app.app_context():
            usuario = Usuario.query.filter_by(correo="noexiste@correo.com").first()
            self.assertIsNone(usuario)

    # 🧪 Prueba 4: Evitar correo duplicado
    def test_usuario_duplicado(self):
        with app.app_context():
            duplicado = Usuario(nombre="Repetido", correo="test@correo.com", contrasena="abcd")
            try:
    ...
    def test_eliminar_usuario(self):
        with app.app_context():
            ...
            if usuario:
                ...
                db.session.commit()
            usuario_borrado = Usuario.query.filter_by(correo="test@correo.com").first()
